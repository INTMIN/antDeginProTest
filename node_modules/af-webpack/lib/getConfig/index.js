...
function _default(opts) {
  ...
  const rule = webpackConfig.module.rule('exclude').exclude.add(/\.json$/).add(/\.(js|jsx|ts|tsx|mjs|wasm)$/).add(/\.(css|less|scss|sass)$/);
  ...
  const babelOptsForDeps = _objectSpread({
  ...
  if (opts.disableDynamicImport) {
    ...
    babelOptsForDeps.plugins = [...(babelOptsForDeps.plugins || []), require.resolve('babel-plugin-dynamic-import-node')];
  ...
  extraBabelIncludes.forEach((include, index) => {
    ...
    webpackConfig.module.rule(rule).test(/\.jsx?$/).include.add(include).end().use('babel-loader').loader(require.resolve('babel-loader')).options(babelOptsForDeps);
  }); // module -> tsx?

  const tsConfigFile = opts.tsConfigFile || (0, _path.join)(opts.cwd, 'tsconfig.json');
